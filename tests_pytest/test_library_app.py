...
class TestBook:
    ...
    def test_cancel_reservation_fail_wrong_reader(self, configure_logger):
        ...
        logger.warning("Cancellation of book reservation fails. Reason: book is reserved by Reader 1. Reader 2 is cancelling.")
    ...
    def test_get_book_fail_reserved_by_another_reader(self, configure_logger):
        ...
        logger.warning("Book cannot be given to Reader 2. Reason: book is reserved by Reader 1.")
    ...
    def test_get_book_fail_already_given(self, configure_logger):
        ...
        logger.warning("Book cannot be given to Reader 1. Reason: book is already given to Reader 1.")
    ...
    def test_return_book_fail_wrong_reader(self, configure_logger):
        ...
        logger.warning("Book cannot be returned by Reader 2. Reason: wrong reader, book is given to Reader 1.")
...
class TestReader:
    ...
    def test_reader_reserve_book_success(self, configure_logger):
        logger = configure_logger
        assert self.reader.reserve_book(self.book) == "0006754023: Success! Reservation for Reader 1 is completed"
    ...
    def test_reader_cannot_reserve_second_book(self, create_book_2, configure_logger):
        ...
        assert self.reader.reserve_book(self.book_2) == "0006754024: Warning! Reader 1 already has a reserved book"
        logger.warning(
            f"{self.reader.name} cannot reserve book {self.book.book_info['book_name']}. "
            f"Reason: {self.reader.name} already has a reserved book: {self.book.book_info['book_name']}")

    def test_reader_reserve_book_fail_book_already_reserved(self, configure_logger):
        logger = configure_logger
        self.book.reserve("Reader 2")
        assert self.reader.reserve_book(self.book) == "0006754023: Warning! The book is already reserved by Reader 2"
        logger.warning(
            f"{self.reader.name} cannot reserve book {self.book.book_info['book_name']}. "
    ...
    def test_reader_cancel_reservation_success(self, configure_logger):
        logger = configure_logger
        self.reader.reserve_book(self.book)
        assert self.reader.cancel_reserve(self.book) == "0006754023: Success! Reservation for Reader 1 is cancelled"
    ...
    def test_reader_cancel_reservation_fail_book_not_reserved_by_reader(self, configure_logger):
        logger = configure_logger
        self.book.reserve("Reader 2")
        assert self.reader.cancel_reserve(self.book) == "0006754023: Warning! Reader 1 has no reservation for the book"
        logger.warning(
    ...
    def test_reader_get_reserved_book_success(self, configure_logger):
        logger = configure_logger
        self.reader.reserve_book(self.book)
        assert self.reader.get_book(self.book) == "0006754023: Success! The book can be given to Reader 1"
        logger.info(f"{self.reader.name} is given reserved book {self.book.book_info['book_name']}")
        assert self.reader.given_book == self.book

    def test_reader_get_not_reserved_book_success(self, configure_logger):
        logger = configure_logger
        assert self.reader.get_book(self.book) == "0006754023: Success! The book can be given to Reader 1"
        logger.info(f"{self.reader.name} is given non-reserved book {self.book.book_info['book_name']}")
        assert self.reader.given_book == self.book

    def test_reader_get_book_fail_reserved_by_another_reader(self, configure_logger):
        logger = configure_logger
        self.book.reserve("Reader 2")
        assert self.reader.get_book(self.book) == "0006754023: Warning! The book cannot be given to Reader 1"
        logger.warning(
    ...
    def test_reader_return_book_success(self, configure_logger):
        ...
        assert self.reader.return_book(self.book) == "0006754023: Success! The book is returned by Reader 1"
        logger.info(f"{self.reader.name} returns book {self.book.book_info['book_name']} successfully.")
    ...
    def test_reader_return_book_fail_wrong_book(self, create_book_2, configure_logger):
        ...
        assert self.reader.return_book(self.book) == "0006754023: Warning! The book could not be returned by Reader 1"
        logger.warning(
